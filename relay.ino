...
void operateRelayBlynk(const String& statusRelay, int setpinOpen, int setpinClose, int blynkValue, int virtualPin, int state) {
...
void controlrelayAuto(){
  // logic_kanopi();
    ...
    operateRelayBlynk("Bergerak Membuka",HIGH, LOW, 1, V2, 1);
    } else if (log_motor == 1 && log_kanopi == 0) {//Closing
    operateRelayBlynk("Bergerak Menutup",LOW, HIGH, 1, V2, 1);
    }  else if (log_motor == 0 && log_kanopi == 0) {//Openend
    operateRelayBlynk("Sudah Terbuka",LOW, LOW, 0, V2, 0);
    } else if (log_motor == 1 && log_kanopi == 1) {//Closed
    operateRelayBlynk("Sudah Tertutup",LOW, LOW, 0, V2, 0);
    }
}
...
void controlrelayOff(){
  operateRelayBlynk("Berhenti ",LOW, LOW, 0, V2, 0);
}
